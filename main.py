# main.py (Versión Final y Consolidada)
...
# --- Importamos TODAS las clases necesarias de nuestros módulos ---
...
# Cada tarea es autocontenida: crea sus propias dependencias para ser más robusta.
...
def tarea_ciclo_principal():
    ...
    try:
        ...
        with DBManager(config.get('db_name')) as db:
            ...
            if datos:
                # El analizador se encarga de todo, incluyendo guardar en la BD
...
def tarea_analisis_pronostico():
    ...
    try:
        ...
        with DBManager(config.get('db_name')) as db:
            ...
            # Para el análisis de pronóstico, no necesitamos el predictor de riesgo actual
            analizador = Analizador(db, notifier, None, config.get('umbrales', {}))
...
def run():
    ...
    try:
        ...
        # Tarea #1: Ciclo principal cada X minutos
        minutos_intervalo = umbrales.get('intervalo_consulta_min', 30)
        scheduler.add_job(tarea_ciclo_principal, 'interval', minutes=int(minutos_intervalo), id='ciclo_principal')
        logger.info(f"Tarea de monitoreo principal programada cada {minutos_intervalo} minutos.")

        # Tarea #2: Análisis de pronóstico cada 6 horas
        ...
        # Tarea #3: Re-entrenamiento del modelo cada domingo a las 3 AM
        ...
        logger.critical(f"Error fatal de configuración en el arranque: {e}")
    except Exception as e:
